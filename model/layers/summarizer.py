...
class PGL_SUM(nn.Module):
    def __init__(self, input_size=1024, output_size=1024, freq=10000, pos_enc=None,
        ...
        self.linear_phi_1 = nn.Linear(in_features=input_size, out_features=input_size//2)
        self.linear_phi_2 = nn.Linear(in_features=input_size//2, out_features=input_size)
    ...
    def forward(self, shot_features, cap_features):
        ...
        zeros = torch.zeros(len(y_ff), 512).to("cuda")  #vivit -> 512
        ...
        video_embedding, new_attn_weights = self.second_attention(y, y)  # Only visual -> (y,y)
        attentive_ft, w = self.second_attention_reconstr(shot_features, shot_features)
        new_attn_weights = torch.unsqueeze(new_attn_weights, dim=0)  # only for visual
        ...
        c = torch.mul(new_attn_weights.t(), y)
        '''
        y = self.drop(y)
        y = self.norm_y(y)
        '''
        ...
        y_regr = self.linear_1(c)
        y_regr = self.relu(y_regr)
        y_regr = self.drop(y_regr)
        y_regr = self.norm_linear(y_regr)

        y_regr = self.linear_2(y_regr)
        y_regr = self.sigmoid(y_regr)
        y_regr = y_regr.view(1, -1)

        # Linear Embedding
        '''
        phi = self.linear_phi_1(c)
        phi = self.relu(phi)
        phi = self.linear_phi_2(phi)
        phi = self.relu(phi)
        '''
        return video_embedding, y_regr  # , phi   # new_attn_weights, attentive_ft
